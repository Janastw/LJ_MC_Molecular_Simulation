...
#include <array> // From read_xyz, for array
#include <vector> // From read_xyz, for vector
#include <utility> // From read_xyz, for pair

using namespace std;

typedef array<double, 3> AtomCoord; // From read_xyz
typedef vector<AtomCoord> Coordinates; // From read_xyz


default_random_engine re; // A Global for Random Number Generator
pair<Coordinates, double> read_xyz(string file_path); // From read_xyz
...
pair<vector<double>, vector<double> > run_simulation(Coordinates coordinates, double box_length, double cutoff, double reduced_temperature, int num_steps, double max_displacement = 0.1, int freq = 1000);
...
int main(void)
{
    ...
    // From read_xyz
    pair<Coordinates, double> xyz_info = read_xyz("../data/sample_config1.txt");
...
double calculate_tail_correction(int n_particles, double box_length, double cutoff)
{
    ...
    double rc3_term = pow(1.0/cutoff, 3);
...
bool accept_or_reject(float delta_e, float beta)
{
   ...
   {
        double random_number = random_double(0.0, 1.0);// USE RANDOM
...
double calculate_pair_energy(Coordinates coordinates, int i_particle, double box_length, double cutoff)
{
    ...
    for (int j_particle = 0; j_particle < num_atoms; j_particle++) 
    {
        if (j_particle != i_particle){
            array<double, 3> j_position = coordinates[j_particle];
...
pair<vector<double>, vector<double> > run_simulation(Coordinates coordinates, double box_length, double cutoff, double reduced_temperature, int num_steps, double max_displacement, int freq)
{
    ...
    vector<double> steps; // Can possibly make a vector and use steps.push_back instead
    vector<double> energies; // Can possibly make a vector and use energies.push_back instead
    ...
    for (int step = 0; step < num_steps; step++)
    {
        ...
        double x_rand = random_double(0.0, 1.0);
        double y_rand = random_double(0.0, 1.0);
        double z_rand = random_double(0.0, 1.0);
        ...
        if ((step % freq) == 0)
        {
            cout << step << ", " << total_energy / num_particles << endl;
...
{           
   ...
   uniform_int_distribution<int> dist(lower_bound, upper_bound-1);
   return dist(re);
}

pair<Coordinates, double> read_xyz(string file_path)
{
    ...
    if(!infile.is_open())
    {   
        throw runtime_error("File path in read_xyz does not exist!");
    }
    ...
    //cout << "Box length: " << box_length << " natoms: " << num_atoms << endl;
    ...
    return make_pair(coords, box_length);
}
